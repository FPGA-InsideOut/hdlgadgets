...
from lib.gadgets.gadget_check_arbiter_class import gadget_check_arbiter
from lib.gadgets.gadget_control_arbiter_class import gadget_control_arbiter
from lib.gadgets.gadget_fplate_sc11_class import gadget_fplate_sc11

class scenario11(scenariotop):
        ...
        def __init__(self):
                ...
                self._check = gadget_check_arbiter()
                self._control = gadget_control_arbiter()
                self._fplate = gadget_fplate_sc11()
        ...
        def setup_2d_model (self, ctx):
                ...
                self._fifo1.setup_2d_model(self._ctx.subwin(2, 2))
                self._fifotr1.setup_2d_model(self._ctx.subwin(2, 2))
                self._fifo2.setup_2d_model(self._ctx.subwin(17, 2))
                self._fifotr2.setup_2d_model(self._ctx.subwin(17, 2))
                self._fifo3.setup_2d_model(self._ctx.subwin(2, 58))
                self._fifotr3.setup_2d_model(self._ctx.subwin(2, 58))
                self._custlogic.setup_2d_model(self._ctx.subwin(5, 48))
                self._modelqueue_a.setup_2d_model(self._ctx.subwin(32, 16))
                self._modelqueue_b.setup_2d_model(self._ctx.subwin(32, 54))
                self._check.setup_2d_model(self._ctx.subwin(32, 92))
                self._control.setup_2d_model(self._ctx.subwin(19, 61))
